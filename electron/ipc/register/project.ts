	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				const trustedExistingProjectPath =
					existingProjectPath &&
					path.extname(existingProjectPath).toLowerCase() ===
						`.${PROJECT_FILE_EXTENSION}` &&
					(isTrustedProjectPath(existingProjectPath) ||
						isPathInsideDirectory(existingProjectPath, projectsDir))
